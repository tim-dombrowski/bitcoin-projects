...
* The [geckor package](https://github.com/next-game-solutions/geckor) is a wrapper for the [CoinGecko API](https://www.coingecko.com/en/api) and allows for easy import of bitcoin price data.
  * Since this package is no longer published to CRAN, we can use `install_github()` to install it directly from its GitHub repo.
...
list.of.packages = c("xts","ggplot2","tseries","seasonal","seasonalview","rmarkdown")
...
So far, we have just used some fairly basic seasonality tools from the `tseries` package. A more in-depth package focused around seasonal adjustments for time series data is the `seasonal` package. After transforming the monthly series into a `seas()` data type, the `view()` command will generate a shiny dashboard that contains many tools for modeling the seasonality of the time series. This dashboard is a local version of [seasonal.website](http://www.seasonal.website/). See the [Introduction to Seasonal](http://www.seasonal.website/seasonal.html) page for more info. The returned output for each of these next two code chunks (one for price series and one for annualized return series) is the result of immediately clicking the "To Console" button in the top-right corner of the dashboard. I've found that if I play around too much before clicking that, there will be some errors that flow through instead of the actual output You will need to actually run this code chunk from RStudio to interact with the dashboard.

```{r seasonal1, message=FALSE, warning=FALSE}
...
From the results above for the annualized return series, we can see that it still identifies November 2013 as the outlier, but then the two MA (moving average) components reduce down to just one AR (autoregressive) component, reflecting the (1 0 0) ARIMA model.


#### Using x13story to Auto-Generate a Story (work-in-progress!)

Another tool around this seasonality package is the auto-generation of a sample R Markdown file to summarize some of the various calculations. This is mentioned on [the GitHub page for the seasonalview package](https://github.com/christophsax/seasonalview) and involves using the [x13story package](https://github.com/christophsax/x13story). This is still a work-in-progress as I've only been able to get it to generate a sample story for the default dataset, not for the BTC prices. Troubleshooting this could be a project idea!

```{r seasonalstory, message=FALSE, warning=FALSE}
#library(x13story)
#view(seas(BTCts_Close), story="story.Rmd")
```
